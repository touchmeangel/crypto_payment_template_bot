...
class ERC20:
    ...
    async def balance_of(self, address: str) -> int:
        ...
        result = await self.instance.functions.balanceOf(
            self.base.web3.to_checksum_address(address)
        ).call()
        return result
    ...
    async def transfers(self, tx_hash: str, to: str) -> list[int]:
        ...
        for log in receipt["logs"]:
            ...
            if AsyncWeb3.to_checksum_address(
                transfer["address"]
            ) == AsyncWeb3.to_checksum_address(
                self.address
            ) and AsyncWeb3.to_checksum_address(
                transfer["args"]["to"]
