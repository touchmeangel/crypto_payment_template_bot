...
async def send_message(
    ...
    telegram_id = (
        message.chat.id if isinstance(message, types.Message) else message.from_user.id
    )
...
def add_pagination_buttons(
    ...
    if unpacked_callback.page > 0:
        ...
        buttons.append(
            types.InlineKeyboardButton(
                text=translations["first"], callback_data=first_page_callback.pack()
            )
        )
        buttons.append(
            types.InlineKeyboardButton(
                text=translations["previous"], callback_data=back_page_callback.pack()
            )
        )
    if unpacked_callback.page < last_page:
        ...
        buttons.append(
            types.InlineKeyboardButton(
                text=translations["next"], callback_data=unpacked_callback.pack()
            )
        )
        buttons.append(
            types.InlineKeyboardButton(
                text=translations["last"], callback_data=last_page_callback.pack()
            )
        )
...
def get_back_to_menu_button(language_code: str):
    ...
    return types.InlineKeyboardButton(
        text="Back", callback_data=create_callback_start(0)
    )


def get_back_to_menu_markup(language_code: str):
    return types.InlineKeyboardMarkup(
        inline_keyboard=[[get_back_to_menu_button(language_code)]]
    )
